...
def _mock_llm() -> MagicMock:
    """LLM that returns a create_reminder tool call then a synthesis message.

    The ReAct loop makes two calls: one to get the tool call, one after the tool
    result is appended to history to get the final answer.
    """
    ...
    synthesis_resp = LLMResponse(
        ...
        content="Reminder set. I'll remind you to call Bob tomorrow.",
        tool_calls=[],
        model="gpt-5.5",
        usage=TokenUsage(input_tokens=20, output_tokens=10, total_tokens=30),
        raw_response_id="resp-integration-2",
    )

    llm = MagicMock()
    llm.embed = AsyncMock(return_value=[[0.0] * 1536])
    llm.complete = AsyncMock(side_effect=[tool_call_resp, synthesis_resp])
    return llm
