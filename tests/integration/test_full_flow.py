...
def pg_url() -> Generator[str, None, None]:
    """Start a pgvector Postgres container and return its connection URL."""
    from testcontainers.postgres import PostgresContainer

    with PostgresContainer("pgvector/pgvector:pg16") as pg:
        # testcontainers gives a psycopg2 URL; swap driver to asyncpg
        url = pg.get_connection_url().replace("psycopg2", "asyncpg", 1)
        # Run alembic migrations against this URL
        result = subprocess.run(
            ["python", "-m", "alembic", "upgrade", "head"],
            env={
                **_base_env(),
                "DATABASE_URL": url.replace("+asyncpg", ""),  # alembic env uses asyncpg internally
            },
            capture_output=True,
            text=True,
        )
...
def _mock_llm(user_id: uuid.UUID) -> MagicMock:
...
async def test_reminder_round_trip(
    ...
    # --- Step 2: assert Memory row was written ---
    async with session_factory() as db:
    ...
    # --- Step 3: assert Reminder row was written ---
    async with session_factory() as db:
    ...
    # --- Step 6: notifier was called with user's telegram_id ---
    mock_notifier.send.assert_called_once_with(db_user.telegram_id, "call Bob")

    # --- Step 7: reminder is now marked sent ---
    async with session_factory() as db:
